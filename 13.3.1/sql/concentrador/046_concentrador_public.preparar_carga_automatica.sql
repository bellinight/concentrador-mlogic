...
SELECT DISTINCT
     ...
     , id_pack_virtual AS id_referencia
...
SELECT DISTINCT
  ...
  JOIN itemPromocao ip
    ON tp.id_referencia = ip.idPromocao
  JOIN item i
    ON i.id = ip.iditem
...
SELECT DISTINCT
  ...
  JOIN promocao_campanha pc
    ON pc.id_campanha = tp.id_referencia
  JOIN item i
    ON i.id = pc.id_item
   AND i.desativado IS NULL
...
SELECT tp.id_referencia
  ...
  JOIN tempPromocao tp ON tp.id_referencia = ppv.id_pack_virtual
