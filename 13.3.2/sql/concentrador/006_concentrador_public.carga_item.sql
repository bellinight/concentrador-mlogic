  ...
  FOR v_registro IN 
    SELECT *
     ...
     WHERE tributacao IS NULL 
        OR unidade IS NULL 
        OR nome IS NULL 
        OR TRIM(both ' ' FROM nome) = ''
        OR preco IS NULL 
     ORDER BY id
  LOOP
    IF (v_registro.id IS NOT NULL) THEN
      IF (v_registro.tributacao IS NULL) THEN
        v_mensagem := v_mensagem || 'Tributação do produto código % não informado.', v_registro.id || chr(10);
      END IF;
      IF (v_registro.unidade IS NULL) THEN
        v_mensagem := v_mensagem || 'Unidade do produto código % não informada.', v_registro.id || chr(10);
      END IF;
      IF (v_registro.nome IS NULL) OR (v_registro.nome = '') THEN
        v_mensagem := v_mensagem || 'Nome do produto código ' || v_registro.id || ' não informado.' || chr(10);
      END IF;
      IF (v_registro.preco IS NULL) THEN
        v_mensagem := v_mensagem || 'Preço do produto código % não informado.', v_registro.id || chr(10);
      END IF;
    END IF;
  END LOOP;
  FOR v_registro IN 
    SELECT *
      FROM ean 
     WHERE iditem IS NULL 
       AND desativado IS NULL
     ORDER BY id
  LOOP
    IF (v_registro.ean IS NOT NULL) THEN
      v_mensagem = v_mensagem || 'EAN ' || v_registro.ean || ' sem item relacionado.' || chr(10);
    END IF;
  END LOOP;
  IF (v_mensagem <> '') THEN
    RAISE EXCEPTION '%', v_mensagem;
  END IF;
